...
#data_pths
#root_path = "/ditto_2"
root_path = "/home/rvsa/ditto_2/"
##realdata_path = os.path.join(root_path, 'real_test/real_datasets_allegro', category)
#pcd_path = os.getcwd()
pcd_path = "/home/rvsa/test_digital/"
results_path = os.path.join(root_path, 'digital_twin')

if not os.path.exists(results_path):
    os.makedirs(results_path)
print(results_path)
print(category+"_"+model_id)
urdf_path = os.path.join(results_path, category + '_' + model_id)
if not os.path.exists(urdf_path):
    os.makedirs(urdf_path)
    
#load in pcd1 and pcd2
pcd_1 = o3d.io.read_point_cloud(os.path.join(pcd_path, 'pcd_drawer_2.pcd'))
pcd_2 = o3d.io.read_point_cloud(os.path.join(pcd_path, 'pcd_drawer_1.pcd'))


ROTATE = category in ['faucet']  # True for drawer and faucet
if ROTATE:
    R = np.array([[-1, 0, 0],
                  [0, -1, 0],
                  [0, 0, 1]])
...
src_pcd = downsample_points(pcd_1, 0.001, 100, 0.1)
dst_pcd = downsample_points(pcd_2, 0.001, 100, 0.1)
#src_pcd = pcd_1
#dst_pcd = pcd_2
...
with initialize(config_path='../configs/'):
    config = compose(
        config_name='config',
        overrides=[
            f'experiment={category.lower()}.yaml',
...
ckpt_path = os.path.join(root_path, args.ckpt_path)
...
#norm_scale = (bound_max - bound_min).max()
...
print("joint_type_prob",joint_type_prob)
...
tri_mesh_1.visual.face_colors = np.array([84, 220, 83, 255], dtype=np.uint8)#mesh1 is the moving part, its green
...
camera_pose = pose
# CAMERA_EXTRINSICS = sapien.Pose(np.array([0.1686067, -0.38366486, 0.7018909]), np.array([0.2729839,-0.8933105,0.3369322, -0.118122]))
# CAMERA_EXTRINSICS_MATRIX = sapien.Pose.to_transformation_matrix(CAMERA_EXTRINSICS)

# camera_pose = CAMERA_EXTRINSICS_MATRIX
light_pose = pose
rgb, depth = renderer.render_mesh(scene, camera_pose, light_pose)
rgb = rgb.copy()

rgb_name = os.path.join(results_path, 'mo3.png') 

cv2.imwrite(rgb_name, rgb)                                       

##  conda activate Ditto
##  cd ditto_2/real_experiment
##  python3 generate_digital_twin.py --category="drawer" --cnt_id=123 --ckpt_path="checkpoints/drawer.ckpt"
##  python3 generate_digital_twin.py --category="laptop" --cnt_id=666 --ckpt_path="checkpoints/Ditto_s2m.ckpt"
