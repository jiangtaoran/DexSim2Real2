        ...
        while num_finished < num:
            ...
            if self.log_every_step and self.num_envs == 1:
                ...
                for key, val in info.items():
                    if key not in runtime_infos:
                        runtime_infos[key] = [] if episode_lens[traj_idx[0]] == 1 else [None] * (episode_lens[traj_idx[0]]-1)
                    runtime_infos[key].append(val)

                self.logger.info(
                    f"Episode {traj_idx[0]}, Stepping {episode_lens[traj_idx[0]]}: Reward: {reward:.3f}, Early Stop or Finish: {done}, Info: {info_str}"
                )
                self.logger.info(str(runtime_infos))
                print("runtime infos:", runtime_infos)
            if self.save_video:
...
class Evaluation:
    ...
    def step(self, action):
        ...
        if self.log_every_step:
            ...
            for key, val in info.items():
                if not key in self.runtime_infos:
                    self.runtime_infos[key] = [] if self.episode_len == 1 else [np.nan] * (self.episode_len - 1)
                self.runtime_infos[key].append(val)
            self.logger.info("Reward function values saved.")
    ...
    def run(self, pi, num=1, work_dir=None, **kwargs):
        ...
        while self.episode_id < num:
            ...
            self.logger.info(f"Episode {self.episode_id} takes {interval:.3f}s.")
            if episode_done:
                reset_pi()
                # log_mem_info(self.logger)
        self.finish()
        try:
            df = pd.DataFrame(self.runtime_infos)
            self.logger.info(f"Saving runtime infos to {osp.join(self.work_dir, 'runtime_infos.xlsx')}")
            df.to_excel(osp.join(self.work_dir, "runtime_infos.xlsx"))
        except:
            traceback.print_exc()
        finally:
            print(self.runtime_infos)
