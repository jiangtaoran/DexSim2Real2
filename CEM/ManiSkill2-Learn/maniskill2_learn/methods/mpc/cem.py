...
class CEMOptimizer:
    ...
    def __call__(self, state, mean=None, std=None):
        ...
        for idx in range(self.n_iter):
            ...
            if hasattr(self, "power_law_sampling") and self.power_law_sampling:
                noise = cn.powerlaw_psd_gaussian(self.power_law_sampling, (x_shape[0], x_shape[2], x_shape[1]))
                noise = np.swapaxes(noise, 1, 2)
                noise = noise.clip(-8, 8)
            elif self.use_trunc_normal:
                noise = trunc_normal(x_shape)
            else:
                noise = np.random.randn(*x_shape)
            start = time()
            samples = noise * _std + mean # (population, horizon, action_dimension)
            ...
            self.logger.info(
                f"CEM round {idx + 1}, action std: {std.max():.3f}, 1-setp-rew: {reward_per_step[:, 0].max() * self.iscost:.3f}, multi-step-avg-rew: {reward.max() * self.iscost:.3f}, time: {interval:.3f}s"
            )
            # self.logger.info(
...
class CEM:
    ...
    def reward(self, state, a):
        ...
        # self.logger.info(f"CEM reward shape: {x.shape}, {a.shape}, rollout.step_states_actions: {self.rollout.step_states_actions}")
        start = time()
        ret = self.rollout.step_states_actions(x, a)[..., 0]
        self.logger.info(f"CEM reward time: {time() - start:.3f}s")
        # sys.exit(0)
