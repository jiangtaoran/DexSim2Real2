...
from mani_skill2.envs.fixed_xmate3_allegro_env import FixedXmate3AllegroEnv
...
class CEMAllegroEnv(FixedXmate3AllegroEnv):
    ...
    def __init__(
        ...
        self.eigengrasp_model = EigenGrasp(16, self.eigen_dim).load_from_file(grasp_model_path)
        self.mean = self.eigengrasp_model._pca.mean_
        self.angle1 = self.eigengrasp_model._pca.components_
    ...
    def _initialize_agent(self):
        #qpos = np.array([-0.488665, 0.340129, -1.14341, 1.18789, 0.346452, 1.73497, -1] + [0.0] * self.eigen_dim)
        #qpos = np.array(list(np.array([0,10,0,91,0,35,0])*np.pi/180) + [0.0]*self.eigen_dim)
        qpos = np.array(list(np.array([0,18,0,53,0,64,0])*np.pi/180) + [0.0]*self.eigen_dim )

        qpos[:-self.eigen_dim] += self._episode_rng.normal(0, 0.02, len(qpos) - self.eigen_dim)

        self._robot_init_qpos = qpos
        self._agent.reset(np.array(list(qpos[:7]) + list(self.eigengrasp_model.compute_grasp(qpos[7:]))))
        #print("初始手角度", self.eigengrasp_model.compute_grasp(qpos[7:]))
        #初始手部角度设为mean的值，即二维量为[0,0]处
    ...
    """
    Deprecated, use compute_dense_reward instead
    """
    @deprecated
    def compute_dense_reward_parallel_gripper(self):
        flag_dict = self.compute_eval_flag_dict()
    ...
    def compute_dense_reward(self):
        def reach_rew():
            # palm pose (location)
            grasp_frame_translation = self.grasp_site.get_pose().p
            #hand_wrist_translation = self.hand_wrist.get_pose().p
        ...
        def contact_reward():
            ...
            return 1 if palm_flag and len(touched_fingers) >= 2 else 0
        ...
        digital_twin_qpos = self._articulation.get_qpos()[self.target_joint_idx]
        contact_direction_rew = -self.accumulated_contact_direction_err
        ...
        # Edit this to change the weight of each reward term
        weight = {
            'reach_weight': 20,
            'contact_weight': 0,
            'progress_weight': 50,
            'penalty_weight': 0.5,
            'goal_achieved_weight': 20,
            "contact_direction_weight":1
        }
        reward_dict = {
            "reach_reward": reach_rew() * weight['reach_weight'],
            "contact_reward": contact_reward() * weight['contact_weight'],
            "progress_reward": progress_reward() * weight['progress_weight'],
            "goal_achieved_reward": self.check_success() * weight['goal_achieved_weight'],
            "penalty": penalty_term() * weight['penalty_weight'],
            "contact_direction_reward":contact_direction_rew*weight["contact_direction_weight"]
        }
        reward = sum(reward_dict.values())
        self.accumulated_contact_direction_err = 0.0
        info_dict = {}
        ...
        try:
            info_dict['grasp_site_pose'] = self.grasp_site.get_pose().p
            info_dict['digital_twin_init_qpos'] = Pose(p=self.digital_twin_init_qpos).p
            info_dict['digital_twin_temp_qpos'] = Pose(self._articulation.get_qpos()[self.target_joint_idx]).p
            self._digital_twin_init_qpos_shower.set_pose(Pose(p=self.digital_twin_init_qpos))
            self._grasp_site_pose_shower.set_pose(self.grasp_site.get_pose())
            self._digital_twin_temp_qpos_shower.set_pose(Pose(self._articulation.get_qpos()[self.target_joint_idx]))
        except:
            pass
        self._cache_info = info_dict
        return reward
